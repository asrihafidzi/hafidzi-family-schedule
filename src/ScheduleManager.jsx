...
export default function ScheduleManager() {
  // 1. KOREKSI BERANDA: Ubah agar TIDAK otomatis baca localStorage, 
  // sehingga saat link dibuka/refresh, selalu muncul halaman pilih nama terlebih dahulu.
  const [currentUserName, setCurrentUserName] = useState('');
  const [inputName, setInputName] = useState('');
  const [nameError, setNameError] = useState('');
  ...
  // State Data dari Firebase
  const [pelajaranData, setPelajaranData] = useState({});
  const [seragamData, setSeragamData] = useState({}); // <--- Sudah diperbaiki di sini
  const [catatanData, setCatatanData] = useState({});
  ...
  useEffect(() => {
    ...
    // 2. KOREKSI SERAGAM: Memperbarui state seragamData agar data dari Firebase terbaca & tampil
    const unsubSeragam = onSnapshot(doc(db, 'family_data', docKeySeragam), (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        setSeragamData(data); // <--- Ini yang sebelumnya kurang
        setItemListSeragam(Array.isArray(data[selectedDay]) ? data[selectedDay] : ['']);
      } else {
        setSeragamData({});
        setItemListSeragam(['']);
  ...
  // ================= HALAMAN LOGIN NAMA (BERANDA UTAMA) =================
  if (!currentUserName) {
    return (
      <div className="min-h-[80vh] flex items-center justify-center px-4">
        <style>{`
